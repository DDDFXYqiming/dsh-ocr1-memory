...
/** 初始化记忆根目录（幂等）：只建目录，不把根伪装成完整 namespace。
 * 实际 namespace 的完整布局（index.txt/facts.md/sops/…）由 ensureNamespaceLayout
 * 在解析出具体 namespace 后执行，避免"幽灵 default"污染根目录。 */
...
function writeMemory(root, { topic, entryType, content, evidence, sourceSession, sourceSeqs, namespace }) {
	...
	// 含换行/控制字符会让 section 解析错位，也成为提示词注入载体，直接拒绝。
	if (/[\n\r\u0000-\u001f\u007f]/.test(safeTopic)) {
