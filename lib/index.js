...
export function apply(ctx, config = {}) {
  ...
  // 主 OCR server 启动后记录的 PID（供 dispose 时停止，避免重载/卸载泄漏进程）。
  let ocrManagedPid = null
  // 自启动任务的 timer 句柄：dispose 时必须清除，否则插件已卸载 timer 仍会触发启动。
  ...
  if (cfg.autoStartOcrServer && cfg.ocrBaseUrl) {
    autoStartTimers.push(setTimeout(() => {
      ensureOcrServer({
        ...
        if (res?.started && res?.pid) ocrManagedPid = res.pid
      }).catch((err) => {
  ...
  if (cfg.autoStartOcrServer && cfg.ocrEmbeddingBaseUrl && cfg.ocrEmbeddingBaseUrl !== cfg.ocrBaseUrl && cfg.ocrEmbeddingAutoStart) {
    autoStartTimers.push(setTimeout(() => {
      ensureOcrServer({
        ...
        if (res?.started && res?.pid && !ocrManagedPid) ocrManagedPid = res.pid
      }).catch((err) => {
  ...
  disposers.push(ctx.effect(() => ctx.tools.register(defineTool({
    ...
    async execute(args) {
      const store = await storePromise
      const result = await store.update(args.id, { text: args.text, source: args.source || null })
  ...
  disposers.push(() => {
    ...
    if (ocrManagedPid) {
      try { process.kill(ocrManagedPid) } catch { /* already stopped */ }
    }
  })
