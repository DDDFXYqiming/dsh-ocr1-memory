...
export const Config = Schema.object({
  ...
  opticalLocatorAutoStart: Schema.boolean().default(false),
  opticalLocatorServerPath: Schema.string().default(''),
  opticalLocatorModelDir: Schema.string().default(''),
  opticalLocatorServerPort: Schema.number().default(18081),
  opticalLocatorModelFile: Schema.string().default('deepseek-ocr-locator-q8_0.gguf'),
  opticalLocatorMmprojFile: Schema.string().default('mmproj-locator-q8_0.gguf'),
  ...
  // 渲染子进程超时与非合并端点下 OCR/locator 服务的上下文容量（原为代码内固定值）。
  renderTimeoutMs: Schema.number().min(5000).default(30000),
  ocrServerContextSize: Schema.number().min(512).default(8192),
  opticalLocatorContextSize: Schema.number().min(512).default(2048),
})

export function apply(ctx, config = {}) {
  // 默认值唯一来源 = Config schema。profile 层把未配置项显式下发为空串，
  // 视同未提供、交给 schema 填默认；schema 表达不了的跨字段/环境变量回退在下方补。
  const provided = {}
  for (const [key, value] of Object.entries(config)) {
    if (value !== '') provided[key] = value
  }
  const cfg = Config(provided)
  cfg.maintenanceBatchSize = Math.max(1, cfg.maintenanceBatchSize)
  cfg.contextMode = cfg.contextMode === 'snapshot' ? 'snapshot' : 'index'
  if (!cfg.opticalLocatorBaseUrl) cfg.opticalLocatorBaseUrl = cfg.ocrBaseUrl
  if (!cfg.ocrEmbeddingBaseUrl) cfg.ocrEmbeddingBaseUrl = cfg.ocrBaseUrl
  if (!cfg.opticalLocatorServerPath) cfg.opticalLocatorServerPath = defaultOcrServerPath()
  if (!cfg.ocrServerPath) cfg.ocrServerPath = defaultOcrServerPath()
  if (!cfg.ocrEmbeddingServerPath) cfg.ocrEmbeddingServerPath = defaultOcrServerPath()
  if (!cfg.opticalLocatorModelDir) cfg.opticalLocatorModelDir = process.env.OPTICAL_LOCATOR_MODEL_DIR || ''
  if (!cfg.ocrModelDir) cfg.ocrModelDir = process.env.OCR_MODEL_DIR || ''
  if (!cfg.ocrEmbeddingModelDir) cfg.ocrEmbeddingModelDir = process.env.OCR_EMBEDDING_MODEL_DIR || process.env.OCR_MODEL_DIR || ''

  mkdirSync(cfg.storeDir, { recursive: true })

  const renderer = cfg.useMockRenderer
    ? createMockRenderer()
    : createRenderer({ python: cfg.pythonPath, renderCommand: cfg.renderScript, timeoutMs: cfg.renderTimeoutMs })
  ...
  function primaryServerSpec() {
    return {
      ...
      contextSize: combinedEndpoint ? cfg.ocrEmbeddingContextSize : cfg.ocrServerContextSize,
    }
  }
  ...
  function locatorServerSpec() {
    return {
      baseUrl: cfg.opticalLocatorBaseUrl,
      port: cfg.opticalLocatorServerPort,
      serverPath: cfg.opticalLocatorServerPath,
      modelDir: cfg.opticalLocatorModelDir,
      modelFile: cfg.opticalLocatorModelFile,
      mmprojFile: cfg.opticalLocatorMmprojFile,
      modelAlias: cfg.opticalLocatorModel,
      embeddings: false,
      contextSize: cfg.opticalLocatorContextSize,
      parallelSlots: 1,
    }
  }
  ...
  if (cfg.opticalLocatorEnabled && cfg.opticalLocatorAutoStart && cfg.opticalLocatorBaseUrl) {
    autoStartTimers.push(setTimeout(() => {
      if (endpointKey(cfg.opticalLocatorBaseUrl) === endpointKey(cfg.ocrBaseUrl)) {
        console.error('[ocr1-memory] optical locator auto-start requires an endpoint distinct from ocrBaseUrl')
        return
      }
      ensureManagedServer(locatorServerSpec()).catch((err) => {
        if (!lifecycle.signal.aborted) console.error(`[ocr1-memory] auto-start optical locator failed: ${err?.message || err}`)
      })
    }, 0))
  }
  ...
  disposers.push(ctx.effect(() => ctx.tools.register(defineTool({
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          opticalLocatorBaseUrl: { type: 'string', required: true },
          opticalLocatorModel: { type: 'string', required: true },
          opticalLocatorAutoStart: { type: 'boolean', required: true },
    ...
    async execute() {
      ...
      return {
        ...
        opticalLocatorBaseUrl: cfg.opticalLocatorBaseUrl,
        opticalLocatorModel: cfg.opticalLocatorModel,
        opticalLocatorAutoStart: cfg.opticalLocatorAutoStart,
        requireOcr: cfg.requireOcr,
