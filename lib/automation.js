...
export function wireAutomation(ctx, cfg, manager) {
  ...
  const scheduleMaintenance = (namespace) => {
    if (maintenanceRuns.has(namespace)) return maintenanceRuns.get(namespace).promise
    const controller = new AbortController()
    const promise = Promise.resolve(manager.maintain({ namespace, signal: controller.signal }))
      .catch((error) => {
        if (!controller.signal.aborted) console.error(`[ocr1-memory] automatic maintenance failed: ${error?.message || error}`)
      })
      .finally(() => {
        maintenanceRuns.delete(namespace)
      })
    maintenanceRuns.set(namespace, { controller, promise })
    return promise
  }
  ...
  return async () => {
    ...
    const pending = []
    for (const { controller, promise } of maintenanceRuns.values()) {
      controller.abort(new Error('ocr1-memory automation disposed'))
