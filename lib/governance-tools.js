  ...
  return [
    ...
    defineTool({
      ...
      async execute(args, exec) {
        return manager.write({ ...args, entryType: args.entry_type, signal: exec?.signal || null })
      },
    }),
    ...
    defineTool({
      ...
      description: '执行去重、L1 压缩、统计和增量光学层级刷新；同一 namespace 不会并发重复运行。',
      timeoutMs: 120_000,
      parameters: { namespace: { type: 'string' } },
      output: {
        schema: { ...objectOutput, properties: {
      ...
      async execute(args, exec) {
        return manager.maintain({ ...args, signal: exec?.signal || null })
      },
    }),
    ...
    defineTool({
      name: 'memory_search',
      description: '全文检索治理记忆（无模型）：覆盖 L2 facts、L3 SOP 和归档条目；可跨 namespace 检索。',
      parameters: {
        query: { type: 'string', required: true },
        namespace: { type: 'string' },
      ...
      output: {
        schema: { ...objectOutput, properties: {
          query: { type: 'string', required: true },
          namespaces_searched: { type: 'array', required: true, items: { type: 'string' } },
          results: { type: 'array', required: true, items: { type: 'object', additionalProperties: true } },
        } },
        render: (_args, value) => [{
          type: 'text',
          text: value.results.length
            ? `检索「${value.query}」（${value.namespaces_searched.join(', ')}）：\n${value.results.map((item, index) => `${index + 1}. [${item.namespace}] ${item.kind}:${item.name}${item.archived ? '（已归档）' : ''} score=${Number(item.score || 0).toFixed(2)}\n   ${item.snippet}`).join('\n')}`
      ...
      execute(args) {
        return manager.search({
          ...args,
          allNamespaces: Boolean(args.all_namespaces),
    ...
    defineTool({
      name: 'memory_promote',
      description: '把项目 namespace 中的 fact/SOP 提升到另一个 namespace（默认 default），源条目默认归档但保留历史。',
      parameters: {
        topic: { type: 'string', required: true },
        entry_type: { type: 'string', enum: ['fact', 'sop'], required: true },
      ...
      output: {
        schema: { ...objectOutput, properties: {
          ...
          source_archived: { type: 'boolean', required: true },
        } },
        render: (_args, value) => [{
          type: 'text',
          text: value.promoted
            ? `✅ 已提升「${value.topic}」：${value.from} → ${value.to}${value.source_archived ? '，源条目已归档' : ''}`
      ...
      async execute(args) {
        return manager.promote({
          ...
          fromNamespace: args.from_namespace,
          toNamespace: args.to_namespace,
          archiveSource: args.archive_source !== false,
        })
      },
    }),
    ...
    defineTool({
      ...
      description: '兼容 layered-memory 的激活入口；当前插件已注册完整治理与光学记忆工具。',
      parameters: {},
      output: {
        schema: { ...objectOutput, properties: {
      ...
      execute() {
        return {
          activated: true,
          tools: [
            'memory_read', 'memory_list', 'memory_write', 'memory_retrieve', 'memory_search', 'memory_index',
            'memory_pending', 'memory_accept', 'memory_update', 'memory_archive', 'memory_promote',
