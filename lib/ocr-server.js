...
// Runtime preference: the CPU-only llama.cpp build so the OCR/embedding
// runtime never touches the discrete GPU. Falls back to the Vulkan build only
// when the CPU build is absent. Set OCR_SERVER_PATH to override explicitly.
const GPU_SERVER = 'D:\\AI_Projects\\models\\llama.cpp\\llama-server.exe'
const CPU_SERVER = 'D:\\AI_Projects\\models\\llama.cpp-cpu\\llama-server.exe'

function resolveDefaultServer() {
  if (process.env.OCR_SERVER_PATH) return process.env.OCR_SERVER_PATH
  if (existsSync(CPU_SERVER)) return CPU_SERVER
  return GPU_SERVER
}
