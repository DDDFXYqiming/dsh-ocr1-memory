...
export function createRenderer({ renderCommand, python = process.env.PYTHON || 'python', timeoutMs = 30_000 } = {}) {
  return async function render(segments, outputPath, { width = 1024, som = true, square = false, signal = null } = {}) {
    if (!renderCommand) {
    ...
    await execFileP(python, [script, payload], { timeout: timeoutMs, maxBuffer: 10 * 1024 * 1024, signal: signal || undefined })
    signal?.throwIfAborted()
...
export function createOcrHttpClient({ baseUrl = 'http://127.0.0.1:8000/v1', apiKey = '', model = 'deepseek-ai/DeepSeek-OCR', timeoutMs = 60_000, repeatPenalty = 0, noRepeatNgramSize = 0 } = {}) {
  return async function transcribe(imagePath, { signal = null } = {}) {
    signal?.throwIfAborted()
...
async function fetchEmbeddingProps({ baseUrl = 'http://127.0.0.1:18080/v1', apiKey = '', timeoutMs = 30_000, signal = null } = {}) {
  signal?.throwIfAborted()
...
export async function measureEmptyPromptTokens({ baseUrl = 'http://127.0.0.1:18080/v1', apiKey = '', model = 'deepseek-ocr', timeoutMs = 30_000, signal = null } = {}) {
  signal?.throwIfAborted()
...
export async function measureImageEmbedding({
  ...
  signal?.throwIfAborted()
  const { readFileSync } = await import('node:fs')
  const props = await fetchEmbeddingProps({ baseUrl, apiKey, timeoutMs: Math.min(timeoutMs, 30_000), signal })
  ...
  if (emptyTokens == null) {
    emptyTokens = (await measureEmptyPromptTokens({ baseUrl, apiKey, model, timeoutMs, signal })).promptTokens
  }
  return {
    embedding,
...
export function createEmbeddingHttpClient({
  ...
  const measure = async function measureImage(imagePath, { signal = null } = {}) {
...
export async function measureTextOnlyPromptTokens({ baseUrl = 'http://127.0.0.1:18080/v1', apiKey = '', model = 'deepseek-ocr', timeoutMs = 30_000, signal = null } = {}) {
  signal?.throwIfAborted()
  const body = {
    model,
    messages: [{ role: 'user', content: '\nFree OCR.' }],
...
export async function createMemoryStore({
  ...
  async function renderEntry(entry, { force = false, tier = null, signal = null } = {}) {
    signal?.throwIfAborted()
    ...
    const run = (async () => {
      ...
      if (!cacheUsed) {
        await renderer(segments, outputPath, { width, som: true, square: true, signal })
        if (useRenderCache) {
          const key = renderCacheKey(entry.segments, width, true)
      ...
      if (entry.visualMemory || embedding) {
        ...
        if (embedding && (!vm.embeddingPath || vm.embeddingPath !== outputPath || vm.embeddingSource !== 'deepseek-ocr-embeddings' || !Array.isArray(vm.embedding) || vm.embedding.length === 0)) {
          try {
            const result = await embedding(outputPath, { signal })
  ...
  async function add({ text, segments = null, source = '', imageExt = 'png', signal = null }) {
    signal?.throwIfAborted()
    await maybeReload()
    ...
    if (source) {
      ...
      if (existing) {
        ...
        await renderEntry(existing, { force: true, tier: 0, signal })
        await save()
  ...
  async function ensureOcr(entry, { signal = null } = {}) {
    ...
    const result = await ocr(entry.imagePath, { signal })
  ...
  async function refreshTiers({ allowedEntryIds = null, limit = Infinity, signal = null } = {}) {
    signal?.throwIfAborted()
    await maybeReload()
    const allowed = Array.isArray(allowedEntryIds)
      ? new Set(allowedEntryIds.map((id) => String(id)))
      : allowedEntryIds
    const cap = Number.isFinite(limit) ? Math.max(0, Math.trunc(limit)) : Infinity
    const nowMs = currentTime()
    let refreshed = 0
    let remaining = 0
    for (const entry of entries) {
      signal?.throwIfAborted()
      if (allowed && !allowed.has(String(entry.id))) continue
      ...
      const stale = entry.tier !== target.name || !entry.imagePath?.includes(`__${target.name}.`) || !entry.imagePath || !existsSync(entry.imagePath)
      if (!stale) {
        entry.tierIndex = idx
        continue
      }
      ...
      await renderEntry(entry, { force: true, tier: idx, signal })
      refreshed++
    }
    signal?.throwIfAborted()
    if (refreshed > 0) await save()
    return { refreshed, remaining, complete: remaining === 0 }
  }
    ...
    if (results === null) {
      ...
      if (prelim.length < topK || requireOcr) {
        ...
        for (const { e } of ranked) {
          ...
          try {
            await ensureOcr(e, { signal: opts.signal || null })
          } catch (err) {
            if (opts.signal?.aborted || requireOcr) throw err
            e.ocrText = ''
          }
          ocrCount++
    ...
    for (const entry of candidateEntries) {
      ...
      if (hitIds.has(entry.id)) {
        ...
        if (entry.tier !== tiers[0].name) {
          ...
          await renderEntry(entry, { force: true, tier: 0, signal: opts.signal || null })
        }
      }
    }
    await save()
  ...
  async function update(id, { text, source = null, signal = null } = {}) {
    signal?.throwIfAborted()
    await maybeReload()
    const entry = entries.find((e) => e.id === id)
    ...
    await renderEntry(entry, { force: true, tier: 0, signal })
    await save()
